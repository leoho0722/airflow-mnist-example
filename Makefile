...
# 透過 docker buildx 建置多架構 Docker Image，並推送到 Docker Hub
.PHONY: build-image
build-image:
ifeq ($(USE_CACHE), false)
	docker buildx build --push --no-cache -f $(DOCKERFILE_PATH) -t $(AUTHOR)/$(IMG):$(IMG_TAG) . --platform $(TARGET_PLATFORM)
else
	docker buildx build --push -f $(DOCKERFILE_PATH) -t $(AUTHOR)/$(IMG):$(IMG_TAG) . --platform $(TARGET_PLATFORM)
endif
